...
//(see also https://en.wikipedia.org/wiki/UVB-76).
// Most of time specific "buzz" noise is being broadcasted, 
//but on very rare occasions, the buzzer signal is interrupted and 
...
function validate(message){
    const rex = new RegExp(/MDZHB\s\d{2}\s\d{3}\s[A-Z]+\s\d{2}\s\d{2}\s\d{2}\s\d{2}$/)
